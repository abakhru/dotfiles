...
use File::Basename;
use FileHandle;
$dir = "/space/amit/scripts/mail";
$host = "greenmeadow.red.iplanet.com";

my $userFrom = "green77";
my $userTo = shift || "green99";
...
sub MSG_SendMessage(@) {
	...
	my	$ToID = "green32";		# The to id
	my	$CCID = "green4";	        # The cc id
	my	$BCCID = "green5";	# The bcc id
...
#	my	$MessageFile = "smfi_header.txt";	# The name of the file containing the body of the message
	...
	foreach $IdItem (@TempArray) {
		...
		$SentMessage{TOSneog} .= "," if ($SentMessage{TOSgreeng});
	...
	foreach $IdItem (@TempArray) {
		...
		$SentMessage{CCSneog} .= "," if ($SentMessage{CCSgreeng});
	...
	foreach $IdItem (@TempArray) {
		...
		$SentMessage{BCCSneog} .= "," if ($SentMessage{BCCSgreeng});
	...
	#	unless ($Smtp->datasend("Cc:  $SentMessage{CCSneog}\n")) {
		unless (($Smtp->datasend("X-Milter-QA1: qa1 header 1\n")) && ($Smtp->datasend("X-Milter-QA2: qa2 header 2\n"))){
			print "Response beginning with 3 not received from a data command: Received\n";
			$Smtp->code();
	...
	#	unless ($Smtp->datasend("Bcc:  $SentMessage{BCCSneog}\n")) {
		unless (($Smtp->datasend("X-Milter-QA3: qa3 header 3\n")) && ($Smtp->datasend("X-Milter-QA4: qa4 header 4\n"))) {
			print "Response beginning with 3 not received from a data command: Received\n";
			$Smtp->code();
			print "Sent:  X-Milter-QA: header 2AMIT \n";
			$Result = 0;
		}
	}
	if ($SentMessage{ContType}) {
		#unless ($Smtp->datasend("Content-type:  $SentMessage{ContType}\n")) {
		unless ($Smtp->datasend("X-Milter-QA: header 3 \n")) {
			print "Response beginning with 3 not received from a data command: Received\n";
			$Smtp->code();
